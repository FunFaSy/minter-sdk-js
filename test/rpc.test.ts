...
test('[RPC] Candidate', async () => {
    ...
    const candidate = await rpcProvider.candidate(validator.public_key);
...
test('[RPC] Coin Info', async () => {
    ...
    const baseCoin = await rpcProvider.coinInfo('BIP');
    const baseCoinById = await rpcProvider.coinInfoById(baseCoin.id);
