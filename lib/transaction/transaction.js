...
class Transaction {
    constructor(data, opts = {}) {
        ...
        if (this.signatureData.length) {
            if (this.isSignatureTypeSingle()) {
                this.signature = new signature_1.SingleSignature(this.signatureData, this);
            }
            //
            else if (this.isSignatureTypeMulti()) {
                this.signature = new signature_1.MultiSignature(this.signatureData, this);
            }
            else {
    ...
    sign(keyPair, multiSigAddress = null) {
        ...
        if (this.isSignatureTypeSingle()) {
            this.signature = new signature_1.SingleSignature(vrsSig, this); // Convert Signature to SingleSignature;
            // Side effect
        ...
        //
            ...
            if (!this.signature || !(this.signature instanceof signature_1.MultiSignature)) {
                this.signature = new signature_1.MultiSignature({ multisig: util_1.toBuffer(multiSigAddress), signatures: [] }, this);
            }
            this.signature.addOne(new signature_1.SingleSignature(vrsSig)); //
