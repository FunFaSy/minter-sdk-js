...
class JsonRpcProvider extends provider_1.Provider {
    ...
    constructor(connection) {
        ...
        if (util_1.isString(connection)) {
            this.connection = { baseURL: connection.toString() };
        }
        //
        else {
    ...
    async address(address, params) {
        if (!address || !util_1.isValidAddress(address)) {
    ...
    async addresses(addresses, params) {
        if (!addresses || !addresses.length || addresses.some(a => !util_1.isValidAddress(a))) {
    ...
    async candidate(publicKey, params) {
        if (!publicKey || !util_1.isValidPublicKey(publicKey)) {
    ...
    async coinInfoById(id, params) {
        if (!util_1.isInteger(Number(id)) || 0 > id) {
    ...
    //----------- Orders
    async limitOrder(orderId, params) {
        if (!util_1.isInteger(Number(orderId)) || 0 > orderId) {
    ...
    async limitOrders(ids, params) {
        if (!ids || !ids?.length || ids.some(id => (!util_1.isInteger(Number(id)) || 0 > id))) {
